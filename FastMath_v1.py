import random
from sympy import *
from incorrect_correct import correct_incorrects_points as c_i_p
    ...
    def multiplication_game(difficulty,amt_problems):
        ...
        if difficulty == easy_mode:
            ...
            for i in range(amt_problems):
                ...
                right_multiplication.append(random.randint(2, 20))
                multiplication_product.append(left_multiplication[i]*right_multiplication[i])

        elif difficulty == hard_mode:
            for i in range(amt_problems):
                left_multiplication.append(random.randint(10,60)) 
                right_multiplication.append(random.randint(2, 60))
                multiplication_product.append(left_multiplication[i]*right_multiplication[i])
        ...
        while c < amt_problems:
            try:
                if value_left[i] > value_right[i]:
                    ...
                    question = float(input(f"Calculate the percentage DECREASE from {value_right[i]} => {value_left[i]} \n>>>"))
                    if question != float(percentage[i]):
                        print(f"incorrect answer, the answer is {value_left[i]} <= {value_right[i]} == {percentage[i]} ")
                        incorrect +=1
                    elif question == float(percentage[i]):
...
#---------#---------#---------#---------#---------#---------#---------#---------#---------#---------#
    ...
    def subtraction_game(difficulty,amt_problems):
        ...
        while c < amt_problems:
            try:
                question = float(input(f"{right_subtraction[i]} - {left_subtraction[i]} = ? \n>>>"))
                if question == (float(subtraction_difference[i])):
                    ...
                    print(f"The answer is: {right_subtraction[i]} - {left_subtraction} = {subtraction_difference[i]}")
                    print("Incorrect answer")
                    incorrect += 1
